...
from Acumenis.app.api.endpoints import requests as api_requests
...
app.mount("/static", StaticFiles(directory=STATIC_DIR), name="static_assets") # Mount assets if needed

# Serve index.html at the root, pricing at /pricing, order at /order
...
async def read_pricing():
    return await serve_static_html("pricing.html")

@app.get("/order", response_class=HTMLResponse, include_in_schema=False)
async def read_order():
    return await serve_static_html("order.html")

# Helper to serve static HTML or fallback
async def serve_static_html(filename: str):
    filepath = os.path.join(STATIC_DIR, filename)
    if os.path.exists(filepath):
        return FileResponse(filepath)
    else:
        # Fallback if MCOL hasn't generated the specific page yet
        return HTMLResponse(f"""
        <html><head><title>Acumenis Backend Running</title></head><body>
        <h1>Autonomous AI Reporting Agency - Backend Active</h1>
        <p>The core engine is running.</p>
        <p>MCOL agent should generate the '{filename}' page shortly.</p>
        <p>Control Panel available at <a href="/ui">/ui</a> | API available at /docs</p>
