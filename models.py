...
from sqlalchemy.dialects.postgresql import ARRAY # Assuming PostgreSQL for ARRAY type
...
class KnowledgeFragment(Base):
    ...
    id = Column(Integer, primary_key=True)
    agent_source = Column(String, nullable=False, index=True) # Agent that created it
    timestamp = Column(DateTime(timezone=True), default=utcnow, nullable=False, index=True) # Creation time
    last_accessed_ts = Column(DateTime(timezone=True), default=utcnow, onupdate=utcnow, nullable=False, index=True) # Last access/update time for pruning
    ...
    item_hash = Column(String(64), unique=True, index=True, nullable=True) # SHA-256 hash of content for deduplication (nullable initially if migrating)
    ...
    related_client_id = Column(Integer, ForeignKey('clients.id'), nullable=True, index=True) # Optional link to a client
...
# --- NEW KB MODELS (Continued) ---

class KnowledgePerformance(Base):
    """Links knowledge fragments to performance metrics."""
    __tablename__ = 'knowledge_performance'
    id = Column(Integer, primary_key=True)
    knowledge_fragment_id = Column(Integer, ForeignKey('knowledge_fragments.id'), index=True, nullable=False) # Renamed FK column
    ...
    # Optional relationship back to the fragment
    # knowledge_fragment = relationship("KnowledgeFragment")
...
class EmailComposition(Base):
    ...
    id = Column(Integer, primary_key=True)
    email_log_id = Column(Integer, ForeignKey('email_logs.id'), unique=True, nullable=False, index=True) # Each email log has one composition
    subject_kf_id = Column(Integer, ForeignKey('knowledge_fragments.id'), nullable=True) # kf = knowledge_fragment
    ...
    # hook_fragment = relationship("KnowledgeFragment", foreign_keys=[hook_kf_id])
    # cta_fragment = relationship("KnowledgeFragment", foreign_keys=[cta_kf_id])
...
class MigrationStatus(Base):
    ...
    migration_name = Column(String, primary_key=True) # e.g., 'encryption_v2'
