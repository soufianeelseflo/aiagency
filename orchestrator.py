...
class Orchestrator:
    ...
    def initialize_database(self):
        if not self.db_pool:
            logger.warning("No DB pool—skipping init.")
            return
        query = """
    ...
    def _get_total_revenue(self):
        ...
        try:
            ...
            logger.error(f"Failed to get revenue: {str(e)}")
            return 0
    ...
    def _count_clients_24h(self):
        ...
        try:
            ...
            logger.error(f"Failed to count clients: {str(e)}")
            return 0

    def _send_status_update(self, message):
        if not self.twilio_client:
            logger.warning("Twilio not configured—update skipped.")
            return
        try:
    ...
    def _extract_phone_number(self, whatsapp_number):
    ...
    def _initiate_voice_call(self):
        if not self.twilio_client:
            logger.warning("Twilio not configured—call skipped.")
            return
        try:
            ...
            logger.info(f"Call to {to_number}: SID {call.sid}")
        except TwilioRestException as e:
            logger.error(f"Failed to call: {str(e)}")
    ...
    def _poll_whatsapp_messages(self):
        ...
        try:
            ...
            logger.error(f"Failed to poll messages: {str(e)}")
            return []

    def _handle_whatsapp_command(self, message, from_number):
        if not self.twilio_client:
            logger.warning("Twilio not configured—command skipped.")
            return
        ...
        logger.info(f"Processed command: '{message}' from {from_number}")
    ...
    def _optimize_strategy(self):
        ...
        if self.db_pool:
            with self.db_pool.getconn() as conn:
                ...
                conn.commit()
                self.db_pool.putconn(conn)
        logger.info(f"Optimized: {strategy}")
        return strategy
    ...
    def _reinvest_profits(self):
        ...
        if profit > 50:
            ...
            logger.info(f"Reinvested ${profit:.2f}")
    ...
    def run(self):
        while self._get_total_revenue() < self.revenue_goal:
            try:
                ...
                logger.error(f"Loop crashed: {str(e)}")
                time.sleep(60)
...
def get_agent_status():
    try:
        ...
        logger.error(f"Agent status failed: {str(e)}")
